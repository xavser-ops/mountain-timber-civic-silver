...
Example root `meta` slice for a game (with cards once published):
...
meta: [
  ...
  ...(ogImage
    ...
    : []),
  // Every app: 50:11 X feed card (x:game:image). Same host guard as og:image.
  // Default og.grok.me/v1/banner.png; games swap to /x-banner.jpg once generated.
  ...(xBanner
    ? [
...
canvas app is missing `og:type="x:game"` or (once the custom link card is
wired) missing `x:game:image` / `public/x-banner.jpg` — treat that as not
done, same as a missing custom card.
...
- **Plain utility apps only** (converters, invoice/CRUD trackers, internal
  ...
  Optional theme colour: append `&color={RRGGBB}` (6-digit hex, no `#` — e.g.
  `FF4D2E`). The service paints it as the card background under the platform
  art, and flips the title and the "Built with Grok" subtitle dark when the
  colour is light. Pick the app's primary/brand colour; omit it to keep the
  default charcoal card.
...
## X feed card: `x:game:image` (50:11)

**Every app wires a second, wider card** that X uses in the feed (the
link-preview `og:image` is still required — this does not replace it).
Declare it with the namespaced property — do **not** invent `x:game:og` or
overload `og:image`. Size is **exactly 50:11** (1200×264 — 1200 is the
standard max card width on the web, same as `og.jpg`). The property name
stays `x:game:image` for non-games too — that is X's feed-card contract,
not a "games only" gate.

**Default (every app, first scaffold): the `og.grok.me` banner**, same
host/title/color as the link card:

```
https://og.grok.me/v1/banner.png?host={VITE_PUBLIC_HOSTNAME}&title={APP_NAME}
```

Optional `&color={RRGGBB}` matches the link card. Wire it in the first
scaffold next to `og:image` — do not wait for a custom file, and do not
gate it on `og:type="x:game"`:
...
Same no-host guard as `og:image`: live preview has no hostname, so emit no
`x:game:image` at all.
...
app is a game**. Non-games keep the `og.grok.me/v1/banner.png` placeholder
forever (same utility/face split does **not** apply here — only games get
Imagine for this asset). Keep a custom file well under 600 KB. Keep
`x:game:image:width` / `x:game:image:height` at `1200` / `264`. Generate
the art at 50:11 (`aspect_ratio: "50:11"` on `imagine_text_to_image` — that
ratio is on the Imagine schema; do **not** substitute 16:9 and crop). Then
normalize:
...
Point `x:game:image` at `"https://${host}/x-banner.jpg"` only once that file
exists (games, brand pass). Until then — and for every non-game — keep the
`og.grok.me/v1/banner.png` placeholder. Never publish a 404ing image URL.
